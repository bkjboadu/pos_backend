...
class TransactionViewTest(APITestCase):
    def setUp(self):
        ...
        self.Product1 = Product.objects.create(name="Milk", barcode="Milk009", price=67)

        self.Transaction1 = Transaction.objects.create(created_by=self.superuser)
    ...
    def test_post_transaction(self):
        # send POST request
        data = {"created_by": self.superuser}
...
class TransactionDetailViewTest(APITestCase):
    ...
    def test_patch_transaction(self):
        ...
        response = self.client.patch(
            f"/sales/transactions/{self.Transaction1.id}/", data
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class TransactionItemViewTest(APITestCase):
    def setUp(self):
        ...
        self.Product1 = Product.objects.create(
            name="Milk", barcode="Milk009", price=67, stock=57
        )

        self.Transaction1 = Transaction.objects.create(created_by=self.superuser)

        self.TransactionItem1 = TransactionItem.objects.create(
            transaction=self.Transaction1, product=self.Product1, quantity=2
        )
...
class TransactionItemDetailViewTest(APITestCase):
    ...
    def test_get_transactionitem_by_id(self):
        ...
        response = self.client.get(
            f"/sales/transactions-items/{self.TransactionItem1.id}/"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_put_transactionitem_by_id(self):
        ...
        response = self.client.put(
            f"/sales/transactions-items/{self.TransactionItem1.id}/", data
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
